# baltimore-value-per-acre-analysis

A way to measure the fiscal sustainability of local land use practices
...
3 - Explore - If you are more into Python, a good place to start is `jupyter lab` from the root of the project, and look in the `./notebooks` directory. If GIS is more your schtick, open the project `./arcgis/baltimore-value-per-acre-analysis.aprx`.
...
- `make env` - builds the Conda environment with all the name and dependencies from `environment_dev.yml` and installs the local project package `baltimore_value_per_acre_analysis` using the command `python -m pip install -e ./src/src/baltimore_value_per_acre_analysis` so you can easily test against the package as you are developing it.
